...
all_u = Vector{Matrix{Float64}}(); all_m = Vector{Vector{Float64}}(); 
# all_r_inv = Vector{Vector{Float64}}(); all_ϵ = Vector{Vector{Float64}}(); 
...
@time for i in range(0, stop = 30, length = 31)
    ...
    # r_inv = zeros(N)
    # for j in 1:N
    #     r_inv[j] = invasion_growth_rate(j, p, x0, tspan, cb)
    # end
...
# Save results
# =============================================================================
results_dir = joinpath(@__DIR__, "../../results/20260511/temp_rich_temperate")
mkpath(results_dir)
@save joinpath(results_dir, "temperate_iters_$(index).jld2") all_rich all_Shannon all_Simpson all_sur all_ext all_u all_m all_Eu all_Em all_Tpu all_Tpm # all_r_inv all_ϵ 