...
│   │   └── MiCRM_demo.ipynb  # Interactive demo notebook
...
```julia
using Pkg
Pkg.add(["Distributions", "Random", "LinearAlgebra", "DifferentialEquations", "Sundials", "Parameters", "CSV", "DataFrames", "CairoMakie", "LsqFit", "Logging", "JLD2", "StatsBase"])
```
...
An interactive demo of the full modelling workflow is available at `code/MiCRM_demo.ipynb`.
...
- **`micrm_params.jl`**: Parameter generation for the MiCRM. Provides default and modular sampling functions for all model parameters: uptake matrix $\mathbf{U}$, leakage tensor $\mathbf{L}$, maintenance costs $m$, resource supply $\rho$, dilution $\omega$, and resource carrying capacities $Kc$. The top-level `generate_params()` assembles these into a single NamedTuple.

  Two sampling schemes are available:
  - **Default (unstructured)**: uptake and leakage drawn from symmetric Dirichlet distributions with no niche structure.
  - **Modular (guild blocks)**: resources 1:M are ordered along a metabolic gradient and partitioned into `N_modules` contiguous guilds. Species are assigned to guilds in order so that the uptake matrix `p.u` plotted as a heatmap directly shows the block structure without sorting. The specialist/generalist axis is controlled entirely by `s_ratio`; leakage follows a thermodynamically irreversible forward chain controlled by `n_byproducts` and `s_ratio_l`.
...
- Smith, T. P., Thomas, T. J., García-Carreras, B., Sal, S., Yvon-Durocher, G., Bell, T. & Pawar, S. (2019), 'Community-level respiration of prokaryotic microbes may rise with global warming', *Nature communications* 10(1), 1–11.
